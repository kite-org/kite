...
Kite supports English and Chinese. Plugins use the SDK's i18n utilities to share this language setting. Plugin dictionaries are isolated from Kite's translations, and plugin text updates when the user changes Kite's language.

## Dictionary Files

Store translations in `src/locales/en.json` and `src/locales/zh.json`. Both files must have **exactly the same key structure**; the build rejects mismatched dictionaries:
